...
(function () {
  ...
  var KNOWN_STATIONS = [
    ...
    { provider:'GEODNET', id:'GNET-SATX1', name:'San Antonio GEODNET',    lat:29.5100, lon:-98.5500, link:'https://ppk.geodnet.com/', color:'#22c55e', tags:['GEODNET','RTK','PPK','RINEX'] },
  ...
  async function getBaseStations(lat, lon, loc) {
    ...
    // Cap per provider so the map stays readable
    var providerLimits = { 'GEODNET':3, 'NOAA CORS':4, 'USGS / NOTA':2, 'Trimble VRS':1 };
    var providerCounts = {};
    var stations = [];
    for (var j = 0; j < scored.length && stations.length < MAX_STATIONS; j++) {
      var st = scored[j];
